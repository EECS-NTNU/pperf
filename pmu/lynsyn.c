...
double const rlDefault[LYNSYN_SENSORS] = {0.025, 0.05, 0.05, 0.1, 0.1, 1, 10};
...
const char *pmuAbout(void) {
  return "Lynsyn PMU, measures current in averaging mode";
}
...
int pmuInit(char *pmuArg) {
  ...
  if ((endPtr == pmuArg) || (sensor < 1) || (sensor > LYNSYN_SENSORS))
    goto pmu_arg_error;
...
pmu_arg_error:
  ...
  return 1;
pmu_init_error:
  fprintf(stderr, "PMU ERROR: could not initialize lynsyn board\n");
  return 1;
}
